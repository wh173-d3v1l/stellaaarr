...
let msg = Config.LANG == 'ES' ? '*¡Este complemento ha sido aprobado oficialmente!* ✅' : '*This plugin has been officially approved!* ✅'
let unmsg = Config.LANG == 'ES' ? '*¡Este complemento no es oficial!* ❌' : '*This plugin is not official!* ❌'
...
var LANG = {
            unaffinfo: Config.LANG == 'ES' ? '*Nivel de peligro del complemento instalado:* _%' : '*Danger Level of Installed Plugin:* _%',
            harmful: Config.LANG == 'ES' ? '*¡Este complemento no se puede instalar porque es dañino!*' : '*This Plugin Cannot Be Installed As It Is Harmful!*',
            duplicate: Config.LANG == 'ES' ? '*¡No puede instalar el mismo complemento dos veces!*' : '*You Cannot Install the Same Plugin 2 Times!*',
            limit: Config.LANG == 'ES' ? '*¡Este complemento excede el límite de seguridad!*\n*Porcentaje de daños:* _%' : '*This Plugin Exceeds Security Limit!*\n*Percentage of Harm:* _%',
            imside: Config.LANG == 'ES' ? '*¡No puede recargar los complementos existentes!*' : '*You Cant Reinstall Existing Plugins!*'
};
DrkBot.addCommand({pattern: 'install ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, warn: Lang.WARN}, (async (message, match) => {

    if (match[1] == '') return await message.client.sendMessage(message.jid,Lang.NEED_URL + '.install https://gist.github.com/BotPrivateDrk/f245b5c5d8d6083f923e6445e0839870', MessageType.text)
    try {
...
DrkBot.addCommand({pattern: 'plugin$', fromMe: true, desc: Lang.PLUGIN_DESC}, (async (message, match) => {
...
DrkBot.addCommand({pattern: 'remove(?: |$)(.*)', fromMe: true, desc: Lang.REMOVE_DESC}, (async (message, match) => {
